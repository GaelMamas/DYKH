...
public class GsonEventFactory {
	 ...
	 * @return 
	 */
	public boolean submitModification(boolean result, String userId, Event event) {
		// TODO Auto-generated method
		return false;
	 }
	...
	public List<Event> getEvents(String userId) {
		...
		if(GenericUsefulMethods.isHistorian(userId).isHistorian()){
			...
			for(Event event: events){
				...
				else if(!TextUtils.isEmpty(event.getUserId()) && event.getUserId().contentEquals(userId)){
					eventsFeeder.add(event);
				}
			}
