...
public abstract class BaseToolbarActivity extends BaseActivity implements Toolbar.OnMenuItemClickListener {
    ...
    protected void beginTransaction(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            fragment = assignFragment(0, savedInstanceState.getInt(Constants.DYKH_FRAGMENT_SELECTION));

        } else {
            fragment = assignFragment(0,0);
        }

        fragment.setArguments(getIntent().getExtras());

        getSupportFragmentManager().beginTransaction().add(R.id.fragment_container_backup, fragment, fragment.getClass().getName()).commit();
    }


    protected abstract BaseFragment assignFragment(int myOwnFragmentIndex, int selection);

    @Override
    protected void onSaveInstanceState(Bundle outState) {
        super.onSaveInstanceState(outState);

        //TODO Make this a little bit clearer
